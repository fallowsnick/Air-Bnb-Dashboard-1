...
st.caption(f"Showing {len(listings)} of {total} total listings")
...
with tab1:
    ...
    for listing in listings:
        ...
        if lat and lng:
            ...
            ✅ Available days: {available} | 📦 Reserved: {reserved}
            """
...
with tab2:
    ...
    for listing in listings:
        ...
        rows.append({
            ...
            "Avg Stay": f"{perf.get('ttm_avg_length_of_stay', 0):.1f} nights",
...
with tab3:
    ...
    selected = st.selectbox("Select a listing", listing_ids)
    ...
    if selected:
        ...
        if detail:
            ...
            st.markdown(f"## {info.get('listing_name')}")
            ...
            col4.metric("Avg Stay", f"{perf.get('ttm_avg_length_of_stay', 0):.1f} nights")

            col5, col6, col7, col8 = st.columns(4)
            col5.metric("Available Days", perf.get("ttm_available_days"))
            col6.metric("Reserved Days", perf.get("ttm_days_reserved"))
            col7.metric("Blocked Days", perf.get("ttm_blocked_days"))
            col8.metric("Total Days Tracked", perf.get("ttm_total_days"))
            ...
            with col_a:
                ...
                st.write(f"**Type:** {info.get('listing_type')}")
                st.write(f"**Bedrooms:** {prop.get('bedrooms')}")
                st.write(f"**Beds:** {prop.get('beds')}")
                st.write(f"**Baths:** {prop.get('baths')}")
                st.write(f"**Guests:** {prop.get('guests')}")
                st.write(f"**Min Nights:** {booking.get('min_nights')}")
            ...
            with col_b:
                st.markdown("### ⭐ Ratings")
                st.write(f"**Overall:** {ratings.get('rating_overall')} ({ratings.get('num_reviews')} reviews)")
                st.write(f"**Cleanliness:** {ratings.get('rating_cleanliness')}")
                st.write(f"**Accuracy:** {ratings.get('rating_accuracy')}")
                st.write(f"**Check-in:** {ratings.get('rating_checkin')}")
                st.write(f"**Communication:** {ratings.get('rating_communication')}")
                st.write(f"**Location:** {ratings.get('rating_location')}")
                st.write(f"**Value:** {ratings.get('rating_value')}")

                st.markdown("### 👤 Host")
                st.write(f"**Name:** {host.get('host_name')}")
            ...
            if amenities:
                ...
                for i, amenity in enumerate(amenities):
                    cols[i % 4].write(f"✓ {amenity.replace('_', ' ').title()}")

            # Last 90 days vs TTM comparison
            st.markdown("---")
            st.markdown("### 📊 TTM vs Last 90 Days")
            compare_df = pd.DataFrame({
                "Metric": ["Revenue", "Avg Rate", "Occupancy", "Avg Length of Stay", "Available Days", "Reserved Days"],
                "Trailing 12 Months": [
                    f"${perf.get('ttm_revenue', 0):,.0f}",
                    f"${perf.get('ttm_avg_rate', 0):,.0f}",
                    f"{perf.get('ttm_occupancy', 0)*100:.1f}%",
                    f"{perf.get('ttm_avg_length_of_stay', 0):.1f} nights",
                    perf.get('ttm_available_days'),
                    perf.get('ttm_days_reserved')
                ],
                "Last 90 Days": [
                    f"${perf.get('l90d_revenue', 0):,.0f}",
                    f"${perf.get('l90d_avg_rate', 0):,.0f}",
                    f"{perf.get('l90d_occupancy', 0)*100:.1f}%",
                    f"{perf.get('l90d_avg_length_of_stay', 0):.1f} nights",
                    perf.get('l90d_available_days'),
                    perf.get('l90d_days_reserved')
                ]
            })
